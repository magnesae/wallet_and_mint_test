...
const abi = [
  ...
  },
  {
    inputs: [
      {
        internalType: 'uint256',
        name: '_maxPurchasePerMint',
        type: 'uint256',
      },
    ],
    name: 'setMaxPurchasePerMint',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
...
const contractAddress = '0x8E6a6dcb916c092C5f5398ED2d1dA90Dcd22d659';
...
async function lazyMint(numberOfTokens: any) {
  const price = 0.2;
  const payPrice = price * parseInt(numberOfTokens);
...
const LazyMint = (props: any) => {
  ...
  const mint = async () => {
    ...
    let res = await lazyMint(numberOfTokens);
    setResult(res);
    setButtonClicked(true);
  };
